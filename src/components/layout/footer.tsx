...
import { cx } from "../../utils/cx.tsx";

function SocialNetworks() {
  const showHighSchool = true;

  if (showHighSchool) {
    return (
      <div className="flex space-x-4 md:justify-center xl:justify-start">
        <a
          href="https://www.instagram.com/borregohighschool/"
          target="_blank"
          rel="noopener noreferrer"
          className="text-gray-400 after:hidden"
          aria-label="Borrego High School's Instagram profile (opens in new window)"
        >
          Borrego High School
        </a>
      </div>
...
export default function Footer() {
  const pathname = "/pathname-no-idea-in-footer"; // usePathname();
  return (
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className=" flex flex-col gap-6 md:flex-row justify-between items-center max-w-7xl px-5 md:px-10 text-center md:text-left external-link">
          <div className="flex flex-col md:flex-row items-center gap-6">
            <SocialNetworks />
            <p className="text-sm">
              &copy; {new Date().getFullYear()} Borrego High School Kids. All
              rights reserved.
            </p>
          </div>
          <p className="text-sm">
            Made with <span className="text-[#ff3838]">♥</span> in Borrego
            Springs by <span className="font-medium">the kids</span>
          </p>
        </div>
      </div>
    </footer>
